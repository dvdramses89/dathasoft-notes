...
3. **SQL crudo solo donde Prisma no llega, y siempre con template tag** — `` prisma.$queryRaw`SELECT ... WHERE id = ${id}` ``, que parametriza de verdad. **Nunca `$queryRawUnsafe`, nunca `$executeRawUnsafe`, nunca concatenar strings** para construir una consulta. Hoy hay exactamente dos: el `` `SELECT 1` `` del health y el ranking del buscador en `documents.service.ts`, que existe porque Prisma no sabe consultar una columna `tsvector`. Ese segundo caso marca el límite: devuelve **solo ids** y filtra por `ownerId`; la proyección y el resto de filtros los hace Prisma. Si necesitas otro, que sea igual de pequeño.
...
6. **Nada de `eval`, `new Function` ni `dangerouslySetInnerHTML`** en el frontend. Hoy no hay ninguno; que siga así.
   - Matiz del único iframe que existe, el embed de YouTube: su `src` **nunca sale de lo que escribió el usuario**. Se guarda el `videoId` ya validado contra `^[A-Za-z0-9_-]{11}$` y la URL se construye a partir de él, así que un `contentJson` manipulado no puede apuntar el iframe a otro sitio. NORMA: **cualquier embed nuevo valida el identificador y compone la URL**; no metas en un `src` una cadena que venga del contenido.
...
- El iframe de YouTube **no pasa por estas cabeceras**: helmet solo cubre las respuestas de la API, y el embed lo pinta el SPA. NORMA: si en el despliegue (Fase 11) el SPA se sirve con CSP propia, esa política necesita **`frame-src https://www.youtube-nocookie.com`**, o el reproductor quedará bloqueado en producción aunque funcione en local.
