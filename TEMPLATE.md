...
| **`.claude/rules/*` y los `CLAUDE.md` de módulo** | 🔶 **Mixto.** Genéricos: `project-workflow`, `git-workflow`, `security`, `backend`, `testing`, la parte A de `frontend` (patrones React) y los `CLAUDE.md` de `auth/` y `prisma/`. Específicos de DTNotes: `database`, la parte B de `frontend` (diseño: tokens del shell, catálogo de clases y tema) y los `CLAUDE.md` de `categories/`, `documents/` y `components/`. La sección **«Librería de componentes: Mantine 8»** de la parte A sí es reutilizable. |
