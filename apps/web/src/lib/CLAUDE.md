...
| `TrashCategory`, `TrashDocument`, `TrashResult`, `TrashSelection` | La papelera. `TrashResult` trae además `retentionDays`, que **lo decide el servidor** |
...
- De la papelera, aquí solo están las **operaciones en lote** (`restoreTrash`, `purgeTrash`) y `emptyTrash`. Las de un elemento existen en la API pero **no tienen función en el cliente**: la UI manda siempre una selección, aunque sea de uno. Si algún día hace falta, se añaden aquí.
